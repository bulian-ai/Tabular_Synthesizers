...
### see https://github.com/gretelai/gretel-synthetics/blob/master/src/gretel_synthetics/utils/stats.py
"""
...
def get_correlation_matrix(df: pd.DataFrame, discrete_columns: List[str] = None):

    if discrete_columns is None:
        discrete_columns = []

    nominal_columns = list(df.select_dtypes(include=["object", "category"]).columns)
    nominal_columns.extend(x for x in discrete_columns if x not in nominal_columns)

    corr_matrix = calculate_correlation(df, nominal_columns=nominal_columns)
    
    return corr_matrix


### Gauge chart ###
### see: https://nicolasfauchereau.github.io/climatecode/posts/drawing-a-gauge-with-matplotlib/

...
class BaseSynthesizerPrivate(nn.Module, metaclass=ABCMeta):
    ...
    def fit_adversarial(self, data,test_pct=0.2,
                        discrete_columns: Union[List, Tuple] = tuple(), ordinal_columns: Union[List, Tuple] = tuple(), \
                            update_epsilon=None, print_freq=50,random_state=42):
        ...
        if test_data.shape[0] < 500:
            warnings.warn(f'{test_data.shape[0]} validation samples detected; consider increasing % allocation in test-split')
        
        print(f"TRAIN SAMPLES: n={len(train_data)}")
        print(f"TEST SAMPLES: n={len(test_data)}")

        self.fit(data=train_data, discrete_columns=discrete_columns, ordinal_columns=ordinal_columns, update_epsilon=update_epsilon,print_freq=print_freq)
    ...
    def sample_adversarial(
        ...
        sampled_data = self.sample(upsampleFrac * n)
    ...
    def load_advarsarial(self, path):
        ...
        print(f"Loading pretrained adversarial from {path} ...")
        # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
        device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
...
class BaseSynthesizer(nn.Module, metaclass=ABCMeta):
    ...
    def fit_adversarial(self, data,test_pct=0.2, epochs=300, discrete_columns=(), print_freq=50,random_state=42):
        self.discrete_columns = discrete_columns
        if not isinstance(data, pd.DataFrame):
            raise TypeError(".{}_adversarial currently supports only pandas.DataFrame")
...
################################################   Base model for Copolla based models #############################################
...
class BaseTabularModel:
    ...
    def _sample_with_conditions(self, conditions, max_tries_per_batch, batch_size,
        ...
        try:
            ...
            print('Provided conditions are not valid for the given constraints')

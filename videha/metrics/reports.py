...
def get_full_report(real_data, synthetic_data,discrete_columns,numeric_columns,target=None,**kwargs):

    ### To do: check no ID columns
    metrics = SingleTableMetric.get_subclasses()

    if target is not None:
        o = compute_metrics(metrics,real_data, synthetic_data,target=target)
    else:
        o = compute_metrics(metrics,real_data, synthetic_data)        
    ...
    # print("\t\t\tDensity Distribution Analysis of Real vs Synthetic Data\n")
    # print("\n")
